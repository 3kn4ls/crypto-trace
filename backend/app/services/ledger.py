...
from app.services.reversal_matching import match_reversals
...
def build_ledger(
    ...
    # Pair each REVERSAL with the reward it claws back so it can inherit that
    # reward's category and zero-cost setting (see reversal_matching.py).
    reversal_map = match_reversals(transactions)
    ...
    for seq, tx in enumerate(transactions):
        ...
        if t == TransactionType.BUY:
            ...
            # gain/loss and back out the income generated by the *matched* reward,
            # using that reward's effective category and zero-cost setting (so a
            # ganancia reward is undone from ganancia, not RCM, and a zero-cost
            # reward — which generated no income — has nothing to back out).
            # Unmatched reversals only remove units and stay PENDING for review.
            # See services/reversal_matching.py and docs/SUGGESTIONS.md.
            if tx.asset_out_id and not _is_fiat(tx.asset_out) and tx.amount_out:
                ...
                reward = tx_by_id.get(reversal_map.get(tx.id))
                if reward is not None and not _reward_zero_cost(reward.taxpayer_id, reward.type):
                    incomes.append(IncomeSpec(tx.id, tx.asset_out_id, tx.timestamp,
                                              -qty, -to_decimal(tx.eur_value),
                                              _reward_category(reward.taxpayer_id, reward.type),
                                              tx.fiscal_year))
