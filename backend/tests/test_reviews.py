...
def test_revert_review_returns_to_pending(client: TestClient):
    ...
    # The reverted P2P item is pending again; nothing remains resolved. (Other
    # categories such as MISSING_PRICE may also be pending, so check P2P here.)
    pending_p2p = client.get(
        "/api/reviews",
        params={"taxpayer_id": tp["id"], "status": "PENDING", "category": "P2P_TRANSFER"},
    ).json()
    assert len(pending_p2p) == 1
    summary = client.get("/api/reviews/summary", params={"taxpayer_id": tp["id"]}).json()
    assert summary["resolved"] == 0


def test_missing_price_autogenerated_on_import_and_self_heals(client: TestClient):
    """Importing an abroad holding without a 31/12 price surfaces a MISSING_PRICE
    item automatically; loading the price auto-resolves it."""
    tp = client.post("/api/taxpayers", json={"name": "Auto Price", "tax_id": "77777777G"}).json()
    acc = client.post("/api/accounts", json={
        "taxpayer_id": tp["id"], "name": "Crypto.com", "platform": "CRYPTO_COM_BANK",
        "type": "EXCHANGE", "is_abroad": True,
    }).json()
    ...
    # No manual /reviews/generate call: the item exists right after the import.
    items = client.get(
        "/api/reviews", params={"taxpayer_id": tp["id"], "category": "MISSING_PRICE"}
    ).json()
    assert len(items) == 1
    assert items[0]["status"] == "PENDING"

    # Loading the 31/12 price reconciles the warning automatically.
    client.post("/api/prices", json={
        "asset_symbol": "CRO", "date": "2024-12-31T00:00:00", "price_eur": "0.1",
    })
    items = client.get(
        "/api/reviews", params={"taxpayer_id": tp["id"], "category": "MISSING_PRICE"}
    ).json()
    assert items[0]["status"] == "RESOLVED"
    assert items[0]["resolution_action"] == "PRICE_LOADED"
